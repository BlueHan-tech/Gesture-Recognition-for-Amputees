...
def main(args):
    ...
    dataset_val = TimeSeriesDataset(data_path=args.val_data_path,
                                    ...
                                    args=args)
    # 🔥🔥🔥 【修正后的调试代码】 🔥🔥🔥
    print(f"\n{'=' * 10} DEBUG DATASET START {'=' * 10}")

    # 1. 强制开启 train 模式以读取标签
    dataset_val.train = True
    print(f"✅ Force set dataset_val.train = {dataset_val.train}")

    # 2. 安全获取样本（不使用解包，避免崩溃）
    spy_sample = dataset_val[2500]
    print(f"📦 Sample[100] returned {len(spy_sample)} items.")

    if len(spy_sample) > 1:
        print(f"🕵️ Spy Label Value: {spy_sample[1]}")  # 这里必须是非0数字！

    # 3. 打印前几个元素，找出哪个是 Label
    # 通常顺序是: [0]Data, [1]Target, [2]Mask, [3]PosEmbed...
    print(f"   Item [0] (Data shape): {spy_sample[0].shape if hasattr(spy_sample[0], 'shape') else 'Not Tensor'}")
    print(f"   Item [1] (Target?): {spy_sample[1]}")
    print(f"   Item [2] (Mask?): {spy_sample[2]}")

    # 4. 自动判定标签是否有效
    # 假设第2个元素(index 1)是标签，这是最常见的情况
    potential_label = spy_sample[1]

    # 如果 index 1 是 tensor 且全是 1 (可能是 mask)，那 label 可能是 index 2
    if isinstance(potential_label, torch.Tensor) and potential_label.numel() > 1:
        print("   (Item 1 looks like a mask or data, checking Item 2...)")
        potential_label = spy_sample[2]
