import os
import warnings, timm; warnings.filterwarnings("ignore", category=FutureWarning, module="timm")
import math
import sys
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, targets_mask, pos_embed_y) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # 如果数据变成了5维 [Batch, 1, 1, H, W]，去掉多余的维度
        if len(samples.shape) == 5:
            samples = samples.squeeze(1)
        # 强制将 target_mask 设为全 1，避免数据被清零
        if targets_mask is not None:
            targets_mask = torch.ones_like(targets_mask, device=device)
...
def evaluate(data_loader, model, device, epoch, log_writer=None, args=None):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # ⚠️⚠️⚠️【关键修改】⚠️⚠️⚠️
        # 绝对不要在分类任务中执行下面这行乘法，它会把整数标签变成浮点数，还会因为mask导致数据损坏
        # target = target * target_mask  <-- 我已经帮你注释掉了

        # 🔥🔥🔥【调试探针】🔥🔥🔥
        # 放在 target 定义之后，Loss 计算之前
        if header == 'Test:' and len(logits) == 0:
            print(f"\n🛑 PROBE TARGET (First 20): {target[:20].cpu().numpy()}")
            unique_targets = torch.unique(target).cpu().numpy()
            print(f"🛑 Unique targets in this batch: {unique_targets}")
            if len(unique_targets) <= 1:
                print(
                    f"⚠️ WARNING: Batch contains only class {unique_targets}. If this is 'val', accuracy will be fake 100%.")
            else:
                print(f"✅ DATA LOOKS GOOD: Multiple classes detected.")
        ...
        with torch.amp.autocast(device_type='cuda'):
            ...
            # 输出层可以乘 mask (为了此时忽略某些预测)，但 target 不能乘
            output = output * target_mask

            # 确保 target 是 Long 类型 (分类任务必须)
            if args.downstream_task == 'classification':
    ...
    if args.downstream_task == 'classification':
        ...
        acc_balanced = 100 * balanced_accuracy_score(y_true=labels, y_pred=logits.argmax(dim=-1))
        try:
            auc = 100 * roc_auc_score(y_true=labels_onehot, y_score=probs, average="macro", multi_class='ovr')
        except Exception:
            auc = 0.0
        auprc = 100 * average_precision_score(y_true=labels_onehot, y_score=probs, average="macro")
        ...
        pcc = np.concatenate([r_regression(logits[:, i].view(-1, 1), labels[:, i]) for i in range(labels.shape[-1])],
                             axis=0)
    ...
    if args.downstream_task == 'classification':
        print(
            '* Acc@1 {top1_acc:.2f} Acc@1 (balanced) {acc_balanced:.2f} Precision {precision:.2f} Recall {recall:.2f} F1 {f1:.2f} AUROC {auroc:.2f} AUPRC {auprc:.2f} loss {losses:.3f}'
            .format(top1_acc=acc, acc_balanced=acc_balanced, precision=precision, recall=recall, f1=f1, auroc=auc,
                    auprc=auprc, losses=test_stats["loss"]))
    elif args.downstream_task == 'regression':
        print('* RMSE {rmse:.3f} MAE {mae:.3f} PCC {pcc:.3f} R2 {r2:.3f} loss {losses:.3f}'
              .format(rmse=test_stats["rmse"], mae=test_stats["mae"], pcc=test_stats["pcc"], r2=test_stats["r2"],
    ...
    if args.wandb == True:
        ...
        if args.plot_embeddings and epoch % 10 == 0:
            ...
            for label in range(args.nb_classes):
                ...
                ax.scatter(umap_proj[indices, 0], umap_proj[indices, 1], c=np.array(cmap(label * 3)).reshape(1, 4),
